...
public class DispatcherServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		System.out.println(path);// application.xml
		try
		{
			...
			WebApplicationContext wc=new WebApplicationContext(temp);
			clsList=wc.getList();
		...
		try {
			...
			//2. 메소드 찾기
				for(String cls:clsList) {
					...
					//컨트롤러 어노테이션 없는경우 제외
					if(clsName.isAnnotationPresent(Controller.class)==false)	continue;
					//메모리 할당
					Object obj=clsName.getDeclaredConstructor().newInstance();
					//메소드 찾기
					Method[] methods=clsName.getDeclaredMethods();
					//모델 안에 선언된 모든 클래스를 가지고 온다, RequestMapping에 해당된 메소드를 찾아 수행
					for(Method m:methods) {
						//어노테이션을 찾아서 밑에 있는 메소드 호출
						RequestMapping rm=m.getAnnotation(RequestMapping.class);
						if(rm.value().equals(cmd)) {
							String jsp=(String)m.invoke(obj, request, response);//obj -> 메모리 할당된 객체
							if(jsp.startsWith("redirect")) {
								//request가 필요없는 경우(jsp로 값을 보내지 않고 초기화)
								response.sendRedirect(jsp.substring(jsp.indexOf(":")+1));
							}
							else {
								//request에 담긴 데이터를 출력할 목적으로 request를 받음 -> forward
			...
			//3. 메소드 호출시 request/session을 jsp로 전송 -> forward
